...
def main(args):
    ...
    for ans in prepro_data['ix_ans'].values():
        if ans not in word_to_ix.keys():
            word_to_ix[ans] = VOCAB_SIZE
            VOCAB_SIZE += 1
    ...
    try:
        ...
        answers[:, 0] = map(convert_to_vocab_ix, qa_data['answers'])
        ...
        for annot_num, annotation in enumerate(train_annotations):
            ...
            if ques_id in ques_id_to_ix.keys():
                ...
                for answer_num, answer in enumerate(annotation['answers']):
                    answers[ques_ix, answer_num + 1] = word_to_ix.get(answer['answer'], UNK)
